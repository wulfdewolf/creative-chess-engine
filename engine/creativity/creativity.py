...
def is_sacrifice(board, move):
    ...
    for capture in captures:
        ...
        if all(
            i.to_square != capture.to_square for i in get_captures(board_after_capture)
        ):
            return True
...
def get_creativity_indices(board):
    ...
    for capture in captures:
        ...
        capture_values.append(
            board.piece_at(get_captured_piece_square(board, capture)).piece_type
        )
    ...
    for move in legal_moves:
        ...
        if board.is_capture(move):
            captured_piece_type = board.piece_at(
                get_captured_piece_square(board, move)
            ).piece_type
