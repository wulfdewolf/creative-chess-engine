    ...
    def play_move(self, current_position):
        ...
        if self.color == current_position.turn:
            ...
            optimality_scores = get_optimality_scores(
                current_position, self.inner_engine
            )
            ...
            hybrid_scores = self.get_hybrid_scores(
                optimality_scores, creativity_indices
            )
            hybrid_scores = sorted(
                hybrid_scores.items(), key=lambda item: item[1], reverse=True
            )

            # Get the top move
            chosen_move = hybrid_scores[0][0]
            chosen_move_creativity_indices = creativity_indices.get(
                chosen_move)

            # Build list that shows what kind of move it is
            indices = [
                weight_index.value for weight_index in chosen_move_creativity_indices
            ]
    ...
    def get_hybrid_scores(self, optimality_scores, creativity_indices):
        ...
        for move in list(optimality_scores) + list(creativity_indices):
            ...
            if move in creativity_indices and move in optimality_scores:
                merged_scores[move] = (
                    optimality_scores[move],
                    creativity_indices[move],
                )
        ...
        for move, score_and_indices in merged_scores.items():
            ...
            hybrid_scores[move] = (
                self.weights[WeightIndex.OPTIMALITY.value] * optimality_score
            )
